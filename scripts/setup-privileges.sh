...
echo "  tee /etc/create_ap.conf + chmod 0600 (owner-only config)"
echo "  killall hostapd dnsmasq"
echo "  nmcli r wifi off|on"
echo "  rfkill unblock wlan"
echo "  ip route add 10.180.0.0/16 via <gw> dev <iface> (portal route with VPN up)"
echo "  sysctl -w net.ipv4.ip_forward=1 (NAT prereq)"
echo "  iptables NAT/forward add|del + read-only list (hotspot NAT)"
echo "  iw dev <iface> station del <mac> subtype 0xC (hotspot kick)"
echo "  wg-quick up|down wg0 (VPN fallback)"
